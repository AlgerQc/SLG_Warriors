...
namespace SLGame
{
    ...
    public class BattleMapData
    {
        ...
        public void Generate(int width, int height, int obstacleCount, int gap)
        {
            ...
            for (int r = 0; r < mapHeight; ++r)
            {
                for (int c = 0; c < mapWidth; ++c)
                {
                    ...
                    gridUnitData.localPosition = new Vector3(
                        c * GameConst.Map_GridWidth + ((r & 1) == (GameConst.Map_FirstRowOffset ? 0 : 1) ? (GameConst.Map_GridWidth * 0.5f) : 0f),
                        -r * GameConst.Map_GridOffsetY,
                        0
                        );
            ...
            Helper.Log(string.Format("Generate map {0}, time cost:{1}", mapID, Helper.TimerEnd()));
        }
        ...
        public GridUnitData GetGridDataByDir(int row, int column, int dir)
        {
            switch (dir)
            {
                ...
                //7点钟方向格子
                case 1:
                    return GetGridData(row + 1, ((row & 1) == (GameConst.Map_FirstRowOffset ? 1 : 0)) ? column - 1 : column);

                //5点钟方向
                case 2:
                    return GetGridData(row + 1, ((row & 1) == (GameConst.Map_FirstRowOffset ? 1 : 0)) ? column : column + 1);
        ...
        public void GetRangeGrids(int row, int column, int range, List<GridUnitData> grids)
        {
            ...
            for (int i = 0; i <= range; ++i)
            {
                ...
                //奇数行开始时
                if ((row & 1) == (GameConst.Map_FirstRowOffset ? 0 : 1))
                {
